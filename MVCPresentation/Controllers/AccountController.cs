    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                LogicLayer.UserManager usrMgr = new LogicLayer.UserManager();   // Explicit path
                try
                {
                    if (usrMgr.FindUser(model.Email))
                    {
                        // This requires the user to use the same password as the one in the internal database
                        var oldUser = usrMgr.LoginUser(model.Email, model.Password);    // This is AuthenticateUser in the other program
                        var user = new ApplicationUser
                        {
                            // Populate the fields with existing data from oldUser
                        ...
                        // Create the user with the Identiry System UserManager normally
                        var result = await UserManager.CreateAsync(user, model.Password);
                        if (result.Succeeded)
                        {
                            // Use the oldUser.Roles list to add the internally assigned roles to the user
                            foreach(var role in oldUser.UserRoles)
                            {
                    ...
                    else // Not an existing user, create a user without roles
                    {
                        var user = new ApplicationUser
                        {
                            // Comment this in when ViewModel and View are updated
        ...
        //
        // GET: /Account/ManualRegisterUser
        [Authorize(Roles = "Administrator")]
        public ActionResult ManualRegisterUser()
        {
            return View();
        }

        //
        // POST: /Account/ManualRegisterUser
        [HttpPost]
        [Authorize(Roles = "Administrator")]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ManualRegisterUser(ManualRegisterUserViewModel model)
        {
            if (ModelState.IsValid)
            {
                // Check to see if this user is in the Database already
                ...
                try
                {
                    ...
                    {
                        var userToRegister = new DataObjects.User()
                        {
                            UserEmail = model.Email,
                            UserName = model.Email
                            // Phone Number doesn't exist for this user object
                        };
                        if (usrMgr.AddUser(userToRegister)) // Add the DataObjects.User to User table
                        {
